...
# Setting path to the XML cascade classifier file for detecting face
...
def detect_and_save_faces_in_place(image_path):
    try:
        ...
        if len(detected_faces) > 0:
            ...
            # Removal of the original image after processing it successfully
            os.remove(image_path)
        else:
            # If no faces are detected, simply delete the photo
            os.remove(image_path)
    except Exception as e:
        print(f"Error processing {image_path}: {str(e)}")

# Traverse through subdirectories and process images
